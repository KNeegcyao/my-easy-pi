...
```mermaid graph TB
    ...
    工具层["③ 工具层 (src/tools/)<br/>ToolRegistry · 7 个内置工具"]
    会话层["② 会话层 (src/session/)<br/>JSONL 持久化 · 会话管理 · 上下文压缩"]
    AI层["① AI 层 (src/ai/)<br/>ModelRegistry · 3 个 Provider · 重试 · 错误码"]
    沙箱层["🐳 沙箱层 (src/sandbox/)<br/>Docker 容器化执行 · 自动降级回退"]
    配置层["⚙️ 配置层 (src/config/)<br/>分层配置管理 · 日志 · 初始化"]

    CLI --> 接口层 --> Agent层 --> 扩展层 --> 工具层 --> 会话层 --> AI层
```
