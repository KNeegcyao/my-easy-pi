...
以下架构图展示了 JSON 模式在 piagent 整体数据流中的位置：

```
┌─────────────────────────────────────────────────────────────────┐
│                        用户 / 调用方                              │
│      (终端脚本 / CI 流水线 / 外部程序 / 日志系统)                  │
└──────────┬──────────────────────────────────────────┬──────────┘
           │                                          │
           │  命令行参数 / stdin                        │  stdout
           │                                          │  JSONL 流
           ▼                                          ▼
┌──────────────────────┐              ┌─────────────────────────┐
│   Agent 核心          │  事件订阅    │   JSON 模式              │
│  (src/agent/loop.ts)  │────────────►│  (src/interface/json.ts) │
│                       │  AgentEvent │                          │
│  处理消息              │             │  JSON.stringify(event)   │
│  调用工具              │             │  + EOL → stdout          │
│  生成回复              │             │                          │
└──────────────────────┘              └─────────────────────────┘
           │                                       │
...
**注意**：以下示例假设 piagent 的 `--output json` 模式已实现。如果当前版本尚未内置，可以通过 RPC 模式或直接调用底层 API 获得类似 JSONL 输出。
