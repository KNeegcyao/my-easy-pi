...
AI 层是整个 piagent 的"底层通信层"，负责屏蔽不同 LLM 提供商（Anthropic、OpenAI、DeepSeek 等）的 API 差异，向上层 Agent 提供统一的流式调用接口。完成本章学习后，你将理解：
...
AI 层是 piagent 的"地基"，它通过四层抽象（类型定义 → Model 接口 → Provider 策略 → 注册中心）将 LLM 调用的复杂性隔离在底层，让上层 Agent 代码可以专注于业务逻辑。此外，统一错误码和重试机制为系统的健壮性提供了保障。
